...
public class SudokuPuzzle {
    ...
    private boolean isBoxesValid() {
        for (int startRow = 0; startRow < getNumRows(); startRow += getBoxHeight()) {
            for (int startCol = 0; startCol < getNumColumns(); startCol += getBoxWidth()) {
                if (!isValidGroup(getBox(startRow, startCol))) {
                    return false;
                }
            }
        }
    ...
    private boolean isValidGroup(String[] group) {
        Set<String> set = new HashSet<>(Arrays.asList(getValidValues()));
        set.removeAll(Arrays.asList(group));
        return set.isEmpty();
    }

    public String[] getColumn(int col) {
        if (inRange(0, col) && inRange(getNumRows() - 1, col)) {
            String[] column = new String[getNumRows()];
            for (int row = 0; row < getNumRows(); row++) {
                column[row] = getValue(row, col);
            }
            return column;
        }
        return new String[0];
    ...
    public String[] getRow(int row) {
        if (inRange(row, 0) && inRange(row, getNumColumns() - 1)) {
            return Arrays.copyOf(board[row], getNumColumns());
        }
        return new String[0];
    }

    public String[] getBox(int startRow, int startCol) {
        if (inRange(startRow, startCol) && inRange(startRow + getBoxHeight() - 1, startCol + getBoxWidth() - 1)) {
            String[] box = new String[getNumRows()];
            int index = 0;
            for (int row = startRow; row < startRow + getBoxHeight(); row++) {
                for (int col = startCol; col < startCol + getBoxWidth(); col++) {
                    box[index++] = getValue(row, col);
                }
            }
            return box;
        }
